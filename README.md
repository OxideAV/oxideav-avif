...
| Item properties                        | `av1C`, `ispe`, `colr` (nclx + ICC), `pixi`, `pasp`, `irot`, `imir`, `clap`, `auxC`, `mdcv`, `clli`, `cclv`, `rloc`, `lsel`, `a1op`, `a1lx`, `iscl` (HEIF §6.5.13 image scaling — four u16 ratio fields + `Iscl::is_well_formed` + `Iscl::scaled_dims`), `rref` (HEIF §6.5.17 required reference types — typed `Vec<BoxType>` + `Rref::{count, requires}`), `crtt` (HEIF §6.5.18 creation time — u64 microseconds since 1904-01-01 UTC + `Crtt::{seconds_since_unix_epoch, subsecond_micros}`), `mdft` (HEIF §6.5.19 modification time — same 1904-epoch microsecond unit as `crtt`; `Mdft::{seconds_since_unix_epoch, subsecond_micros}` mirror the `crtt` helpers, and `mdft` may legally co-occur with `crtt` on a single item to surface a creation/modification pair), `udes` (HEIF §6.5.20 user description — four UTF-8 strings `lang` / `name` / `description` / `tags` with the §6.5.20.3 empty-string "absent" sentinels projected via `Udes::{lang_opt, name_opt, description_opt, tags_opt}` and a `Udes::tag_list` view that splits on `','` and trims each segment; §6.5.20.1 quantity is zero-or-more so multiple language variants legally co-occur on a single item), `altt` (HEIF §6.5.21 accessibility text — FullBox + two UTF-8 strings `alt_text` then `alt_lang` in the §6.5.21.2 declaration order, reversed relative to `udes`; §6.5.21.3 empty `alt_lang` is the unknown/undefined sentinel projected via `Altt::{alt_text_opt, alt_lang_opt}`; §6.5.21.1 quantity is zero-or-more so multiple language variants of the alternate text legally co-occur on a single item), `aebr` (HEIF §6.5.22 auto-exposure information — FullBox + two `int(8)` fields `exposure_step` then `exposure_numerator` per §6.5.22.2; the §6.5.22.3 enumeration for `exposure_step` is exposed via `Aebr::{STEP_FULL, STEP_HALF, STEP_THIRD, STEP_QUARTER}` constants + `Aebr::is_defined_step`, and the stops offset (`exposure_numerator / exposure_step` per §6.5.22.3) via `Aebr::exposure_stops` returning `None` for the reserved zero step; both fields are signed so a negative numerator round-trips correctly as a darker-than-camera bracket position; §6.5.22.1 quantity is at-most-one), `wbbr` (HEIF §6.5.23 white-balance information — FullBox + `unsigned int(16)` `blue_amber` (colour-temperature component in Kelvin, big-endian per ISO/IEC 14496-12 §4.2) + signed `int(8)` `green_magenta` (colour-deviation component in 1/100 Duv) per §6.5.23.2; the §6.5.23.3 NOTE describes `green_magenta == 0` as the neutral sentinel, with negative = magenta shift and positive = green shift, surfaced via `Wbbr::NEUTRAL_GREEN_MAGENTA` + `Wbbr::is_green_magenta_neutral` + the `Wbbr::green_magenta_duv` projection that divides the wire field by 100.0 so callers don't re-derive the unit conversion; §6.5.23.1 quantity is at-most-one), `fobr` (HEIF §6.5.24 focus information — FullBox + `unsigned int(16)` `focus_distance_numerator` + `unsigned int(16)` `focus_distance_denominator` per §6.5.24.2; focus distance in metres is the ratio of the two fields per §6.5.24.3, with **focus at infinity** signalled by the divide-by-zero sentinel `focus_distance_denominator == 0` (and the numerator `should` also be zero per the same paragraph); surfaced via `Fobr::INFINITY_DENOMINATOR` constant + `Fobr::focus_distance_metres` returning `Option<f64>` (`None` on infinity), `Fobr::is_focus_at_infinity` (permissive — denominator zero alone, matching the spec `i.e.` clause), and `Fobr::has_well_formed_infinity_sentinel` (strict — both fields zero, distinguishing writers that honour the `should`); §6.5.24.1 quantity is at-most-one), `afbr` (HEIF §6.5.25 flash-exposure information — FullBox + two `int(8)` (signed) fields `flash_exposure_numerator` then `flash_exposure_denominator` per §6.5.25.2; the flash exposure value of the sample expressed in **number of f-stops** is the ratio of the two fields per §6.5.25.3, surfaced via `Afbr::flash_exposure_stops` returning `Option<f64>` (`None` on the spec-undefined zero-denominator case — §6.5.25 does NOT carve out an infinity sentinel like `fobr`'s §6.5.24.3, so a zero denominator is mathematically undefined and surfaces as `None` mirroring the `aebr` / `Aebr::exposure_stops` pattern); fields are signed so a negative numerator round-trips correctly as an under-exposed (darker) flash bracket position and a `0xFF` byte reads as `-1`, not `255`; `i8::MIN / -1` round-trips as `128.0` via the f64 widening so an integer-overflow panic is impossible; §6.5.25.1 quantity is at-most-one), `dobr` (HEIF §6.5.26 depth-of-field information — FullBox + two `int(8)` (signed) fields `f_stop_numerator` then `f_stop_denominator` per §6.5.26.2; the depth-of-field variation is expressed as an aperture change in a **number of stops** as the ratio of the two fields per §6.5.26.3, surfaced via `Dobr::aperture_stops` returning `Option<f64>` (`None` on the spec-undefined zero-denominator case — §6.5.26, like §6.5.25 `afbr`, does NOT carve out an infinity sentinel, so a zero denominator is mathematically undefined and surfaces as `None`); structurally identical to the `afbr` flash-exposure sibling — fields are signed so a negative numerator round-trips as a shallower-depth-of-field bracket position and a `0xFF` byte reads as `-1`, not `255`; `i8::MIN / -1` round-trips as `128.0` via the f64 widening so an integer-overflow panic is impossible; §6.5.26.1 quantity is at-most-one), `pano` (HEIF §6.5.27 panorama information — FullBox + `unsigned int(8)` `panorama_direction` and, **only** for the two grid directions (`4` raster scan / `5` continuous order), a conditional `unsigned int(8)` `rows_minus_one` + `columns_minus_one` pair per §6.5.27.2 — the first conditionally-sized body in the §6.5.x rollout, surfaced as `Pano::grid: Option<PanoGrid>`; the six §6.5.27.3 directions are exposed via `Pano::DIRECTION_*` constants + `is_defined_direction` / `is_grid`, undefined values `>= 6` parse cleanly with the raw byte preserved; `PanoGrid::{rows, columns}` add the minus-one back with a `u16` widening so wire `255` reads as `256`; per §6.5.27.1 quantity is at-most-one and the property `should` only be associated with a `'pano'` entity group (§6.8.8.1) — classified via the new `EntityGroup::is_panorama()`), `subs` (HEIF §6.5.28 sub-sample information — the one §6.5.x property defined by reference to ISO/IEC 14496-12's `SubSampleInformationBox` (§8.7.7.2) rather than self-contained in the HEIF spec; HEIF §6.5.28 fixes the outer table to a single degenerate row (`entry_count == 1`, `sample_delta == 0`, both enforced) so the parser surfaces only the inner sub-sample list as `Subs::entries: Vec<SubsEntry>` — each `(subsample_size, subsample_priority, discardable, codec_specific_parameters)` with `subsample_size` 32-bit for box `version==1` else 16-bit, widened to `u32` so callers need not branch on the wire width; box `flags` surfaced via `Subs::flags` since §6.5.28 permits zero-or-more `subs` per item with differing `flags`; empty `subsample_count` is well-formed, v0/v1 accepted and other versions rejected), `tols` (HEIF §6.5.29 target output layer set — `ItemFullProperty('tols', version=0, flags=0)` + a single big-endian `unsigned int(16) target_ols_idx` per §6.5.29.2, surfaced as `Tols::target_ols_idx`; the field is the output layer set index handed to the decoding process of the associated coded image item, coding-format-specific per §6.5.29.3 so surfaced verbatim; uniquely among the §6.5.x descriptive family the spec *requires* it essential (§6.5.29.1 `essential shall be equal to 1`), so surfacing the typed value is what keeps a `tols`-carrying item off `Meta::unsupported_essential_properties`), `prdi` (HEIF §6.5.37 progressive-derived-image-item information — `ItemFullProperty('prdi', version=0, flags)` whose **entire body is flags-gated** per §6.5.37.2: `step_count` present iff `one_item_per_step` clear OR `alternative_is_candidate` set, and the per-step `item_count[]` array present iff `one_item_per_step` clear; the three §6.5.37.1 flag bits surfaced via `Prdi::{FLAG_ITEM_REFERENCE_ORDER, FLAG_ONE_ITEM_PER_STEP, FLAG_ALTERNATIVE_IS_CANDIDATE}` + `is_*` projections with the whole 24-bit flags field preserved; `Prdi::{step_count, item_counts}` are `Option` (`Some` exactly when present on the wire) and the §6.5.37.3 inference rule is applied by `Prdi::{step_count_or_inferred(dimg_input_count), item_count_for_step(i)}` — inferring `step_count` from the `'dimg'` input count and `item_count == 1` per step in the empty-body flag combination; descriptive so a recognised association never trips `Meta::unsupported_essential_properties`), `wipe` / `zoom` / `fade` / `splt` / `stpe` / `ssld` (HEIF §6.5.30–§6.5.35 slideshow transition-effect family — `wipe` (eight §6.5.30.3 directions via `Wipe::FROM_*` + `is_known_direction`), `zoom` (the §6.5.31.2 single octet unpacked into `Zoom::{transition_direction (top 1 bit), transition_shape (low 7 bits)}` with `DIRECTION_{IN,OUT}` / `SHAPE_{RECTANGULAR,CIRCLE,STAR}` + `is_known_shape`), `fade` (`THROUGH_WHITE` / `THROUGH_BLACK` / `DISSOLVE`), `splt` (`VERTICAL_{IN,OUT}` / `HORIZONTAL_{IN,OUT}`), `stpe` (`transition_period` in 1/16 s + `Stpe::seconds`), `ssld` (`duration` in 1/16 s + `Ssld::{seconds, is_reserved}` for the §6.5.35.3 `duration == 0` reserved sentinel); each associated with the **first** of two consecutive slideshow items; though §6.5.30–§6.5.33 list the effects as transformative they are presentation hints not pixel transforms, so a recognised association never trips `Meta::unsupported_essential_properties`; unknown `version` rejected, reserved enumerants surfaced verbatim, trailing bytes ignored), `cmin` (HEIF §6.5.40 camera intrinsic matrix — `ItemFullProperty('cmin', version=0, flags)` with three mandatory `signed int(32)` fields (`focal_length_x`, `principal_point_x`, `principal_point_y`) plus, **only** when `flags & 1` (full intrinsics per §6.5.40.3), a `signed int(32) focal_length_y` + `signed int(32) skew_factor` tail; the simplified form leaves both `None` with the §6.5.40.3 inferences `fy = fx` / `s = 0` applied by the projection helpers; the two 5-bit shift operands embedded in `flags` are decoded per §6.5.40.1 (`denominator = 1 << ((flags & 0x001F00) >> 8)`, `skewDenominator = 1 << ((flags & 0x1F0000) >> 16)`) and surfaced via `Cmin::{denominator_shift, skew_denominator_shift, denominator, skew_denominator, has_skew}` + `Cmin::FLAG_FULL_INTRINSICS`; the §6.5.40.1 matrix-entry formulas folding in the associated `ispe` dimensions are applied by `Cmin::{focal_length_x_value, focal_length_y_value, principal_point_x_value, principal_point_y_value, skew_value}` (floating-point division per NOTE 3); all five fields signed so a negative principal point / skew round-trips; §6.5.40.1 quantity is at-most-one); unknown boxes retained as `Property::Other` so indices stay valid |
...
### Round 309 — HEIF §6.5.40 `cmin` camera-intrinsic-matrix item property

Advances the §6.5.x descriptive item-property rollout to §6.5.40
CameraIntrinsicMatrixProperty, the camera-side counterpart to the
§6.5.39 extrinsic matrix (whose `version==1` form defers to
`ViewpointGlobalCoordinateSysRotationStruct` in ISO/IEC 23090-7,
absent from the staged docs). `cmin` describes the pinhole-camera
intrinsic matrix
...
of the camera that captured the associated image item. The wire
layout is taken verbatim from §6.5.40.2 — an
ItemFullProperty(`cmin`, version=0, flags) with three mandatory
`signed int(32)` fields (`focal_length_x`, `principal_point_x`,
`principal_point_y`) followed, **only** when the low `flags` bit is
set (full intrinsics per §6.5.40.3), by a `signed int(32)
focal_length_y` + `signed int(32) skew_factor` tail:
...
The simplified form (`flags & 1 == 0`, no skew / square pixels)
leaves `focal_length_y` / `skew_factor` as `None`; the §6.5.40.3
inferences (`fy = fx`, `s = 0`) are applied by the projection
helpers. Two 5-bit power-of-two shift operands embedded in `flags`
select the denominators per §6.5.40.1
(`denominator = 1 << ((flags & 0x001F00) >> 8)`,
`skewDenominator = 1 << ((flags & 0x1F0000) >> 16)`), decoded via
`Cmin::{denominator_shift, skew_denominator_shift, denominator,
skew_denominator}`; the whole 24-bit `flags` field is preserved.
The §6.5.40.1 matrix-entry formulas (which fold in the
`image_width` / `image_height` from the associated `ispe`) are
applied by `Cmin::{focal_length_x_value, focal_length_y_value,
principal_point_x_value, principal_point_y_value, skew_value}`,
each returning the floating-point matrix entry (floating-point, not
integer, division per NOTE 3). All five fields are reinterpreted as
`i32` so a negative principal point or skew round-trips. Unknown
`version` rejected, a body short of the three mandatory fields (or,
for the full form, the two-field tail) rejected, trailing bytes
ignored; descriptive per §6.5.40.1 so a recognised `cmin` never
trips `Meta::unsupported_essential_properties`.

Tests: +13 unit (simplified / full round-trip, signed fields, the
flags-derived denominators, the matrix-entry projections, the
`fy = fx` / `s = 0` simplified inferences, the skew formula,
short/unknown-version/truncated-tail rejection, trailing-byte
tolerance, `ipco` dispatch, the not-essential-blocking guarantee,
and `property_for` lookup). Default lib 499 (was 486); standalone
lib 484. Re-export: `oxideav_avif::Cmin`.

Followups: §6.5.39 `cmex` camera extrinsic matrix — its `version==0`
quaternion form is fully self-contained in the staged spec, but the
`version==1` form defers to `ViewpointGlobalCoordinateSysRotationStruct`
(ISO/IEC 23090-7), not in docs/. §6.5.38 `sstr` SingleStreamProperty
(empty `ItemProperty('sstr')`) remains. §6.5.36 `amve` stays
docs-blocked (#1668).

### Round 306 — HEIF §6.5.30–§6.5.35 slideshow transition-effect item-property family

Lands six item properties in one coherent increment — the slideshow
transition-effect family fully self-contained in the staged
third-edition HEIF spec text. Each documents a suggested presentation
behaviour between two consecutive items of a slideshow entity group and
is associated with the **first** of the two items involved; the precise
rendering is deliberately left to the player.

- §6.5.30 `wipe` → `Property::Wipe(Wipe { transition_direction })`.
  Eight defined directions (§6.5.30.3) as `Wipe::FROM_LEFT …
  FROM_RIGHT_BOTTOM` constants; `Wipe::is_known_direction` separates the
  reserved `>= 8` range, surfaced verbatim.
- §6.5.31 `zoom` → `Property::Zoom(Zoom { transition_direction,
  transition_shape })`. The §6.5.31.2 body packs both fields into a
  single octet — `unsigned int(1) transition_direction` (top bit) then
  `unsigned int(7) transition_shape` (low seven) — unpacked at parse
  time. `Zoom::{DIRECTION_IN, DIRECTION_OUT}` +
  `Zoom::{SHAPE_RECTANGULAR, SHAPE_CIRCLE, SHAPE_STAR}` constants;
  `Zoom::is_known_shape`.
- §6.5.32 `fade` → `Property::Fade(Fade)` with
  `Fade::{THROUGH_WHITE, THROUGH_BLACK, DISSOLVE}`.
- §6.5.33 `splt` → `Property::Splt(Splt)` with
  `Splt::{VERTICAL_IN, VERTICAL_OUT, HORIZONTAL_IN, HORIZONTAL_OUT}`.
- §6.5.34 `stpe` → `Property::Stpe(Stpe { transition_period })`, the
  recommended transition period in units of 1/16 s; `Stpe::seconds`
  applies the §6.5.34.3 unit.
- §6.5.35 `ssld` → `Property::Ssld(Ssld { duration })`, the recommended
  display duration in units of 1/16 s; `Ssld::seconds` converts and
  `Ssld::is_reserved` flags the §6.5.35.3 reserved `duration == 0`
  sentinel.

Although §6.5.30–§6.5.33 list the effects as *transformative*, they are
slideshow-presentation hints, not pixel transforms — so a recognised
association never trips `Meta::unsupported_essential_properties` (the
crate's essential-property model treats any typed property as
recognised). Forward-compatibility matches the rest of the module:
unknown `version` rejected, truncated `ssld` duration rejected, trailing
bytes ignored, reserved enumerants surfaced verbatim.

Tests: +19 unit covering every defined enumerant, the `zoom` bit-pack
unpacking, reserved-value handling, the 1/16-s conversions, short/
unknown-version rejection, `ipco` dispatch, the
not-essential-blocking guarantee, and `property_for` lookup. Default
lib 486 (was 467). Re-exports: `oxideav_avif::{Wipe, Zoom, Fade, Splt,
Stpe, Ssld}`.

Followups: §6.5.38 `sstr` (SingleStreamProperty), §6.5.39 / §6.5.40
camera extrinsic (`cmex`) / intrinsic (`cmin`) matrices remain in the
staged third-edition PDF; §6.5.36 `amve` stays docs-blocked (#1668).

### Round 300 — HEIF §6.5.37 `prdi` progressive-derived-image-item-information item property

Advances the §6.5.x descriptive item-property rollout past the
ISO/IEC-14496-12-referenced `subs` (§6.5.28) and the
docs-blocked-body `amve` (§6.5.36, whose syntax defers to the
`AmbientViewingEnvironmentBox` absent from the staged 14496-12 PDFs —
issue #1668) to land §6.5.37
`ProgressiveDerivedImageItemInformationProperty`, the next property
fully self-contained in the staged third-edition HEIF spec text.

`prdi` describes the progressive rendering steps associated with a
**derived** image item (§6.5.37.1): each step specifies the number of
candidate input image items to use for the reconstruction, described
as a difference from the previous step. Quantity per item is
zero-or-one for a derived image item.

It is the first §6.5.x property whose body is **entirely gated by the
box `flags`** — there is no unconditionally-present payload byte. The
wire layout (§6.5.37.2):
...
Three `flags` bits are defined (§6.5.37.1) and surfaced as
`Prdi::{FLAG_ITEM_REFERENCE_ORDER (0x1), FLAG_ONE_ITEM_PER_STEP (0x2),
FLAG_ALTERNATIVE_IS_CANDIDATE (0x4)}` with `is_*` projections; the
whole 24-bit field is preserved so an unknown future bit round-trips.
The parser mirrors the three resulting body shapes exactly:

- `one_item_per_step` clear → `step_count` present, `item_count[]`
  present (read with exactly `step_count` entries).
- `one_item_per_step` set + `alternative_is_candidate` set →
  `step_count` present (no longer derivable from `'dimg'` once
  alternatives extend the candidate list), `item_count[]` absent.
- `one_item_per_step` set + `alternative_is_candidate` clear → empty
  body; both values inferred per §6.5.37.3.

`Prdi::{step_count, item_counts}` are therefore `Option`, `Some`
exactly when present on the wire. The §6.5.37.3 inference rule is
applied by `Prdi::step_count_or_inferred(dimg_input_count)` (inferring
the `'dimg'` input count) and `Prdi::item_count_for_step(i)`
(inferring `item_count == 1` per step). A zero `step_count` (reserved
for future use) parses cleanly as an empty array, surfaced verbatim.
Forward-compatibility matches the rest of the module: unknown
`version` rejected, truncated `step_count` / short `item_count[]`
array rejected, trailing bytes ignored. Descriptive per §6.5.37.1, so
a recognised `prdi` — even when flagged essential — never trips
`Meta::unsupported_essential_properties`.

Tests: +11 unit (`prdi_full_form_reads_step_count_and_item_counts`,
`prdi_one_item_per_step_with_alternatives_keeps_step_count_only`,
`prdi_one_item_per_step_no_alternatives_has_empty_body`,
`prdi_item_reference_order_flag_round_trips`,
`prdi_zero_step_count_yields_empty_item_counts`,
`prdi_rejects_unknown_version`, `prdi_rejects_truncated_body`,
`prdi_tolerates_trailing_bytes`, `prdi_dispatched_through_parse_ipco`,
`prdi_essential_association_is_recognised`,
`prdi_lookup_via_property_for`). Default lib 467 (was 456);
standalone lib 452 (was 441); integration unchanged. Re-export:
`oxideav_avif::Prdi`.

Followups: §6.5.38 `sstr` (SingleStreamProperty — an empty
`ItemProperty('sstr')` flagging that a derived item's input data form
a single concatenated bitstream; the README's earlier `slst`
shorthand was a typo — the staged spec box type is `'sstr'`).
§6.5.39 / §6.5.40 camera extrinsic (`cmex`) / intrinsic (`cmin`)
matrices remain in the staged third-edition PDF. §6.5.36 `amve`
stays docs-blocked (#1668).

### Round 293 — HEIF §6.5.28 `subs` sub-sample-information item property

Backfills the §6.5.x item-property gap the round-289 note flagged:
§6.5.28 SubSampleInformationBox, the one descriptive property defined
by reference to ISO/IEC 14496-12 rather than self-contained in the
HEIF spec text. For coded image items the property body is exactly a
`SubSampleInformationBox` (ISO/IEC 14496-12 §8.7.7.2) for the coding
format of the associated item.

HEIF §6.5.28 pins the otherwise-variable outer table to a single
degenerate row: `entry_count` shall be `1` and that entry's
`sample_delta` shall be `0`. The parser enforces both and surfaces
only the inner sub-sample list — the entry's `subsample_count` loop
body — as `Subs::entries: Vec<SubsEntry>`:

```text
unsigned int(16|32) subsample_size;   // 32-bit when version==1, else 16-bit
unsigned int(8)     subsample_priority;
unsigned int(8)     discardable;
unsigned int(32)    codec_specific_parameters;
```

`subsample_size` widens to `u32` regardless of box version so a caller
need not branch on the wire width. The box `flags` are surfaced via
`Subs::flags`: §6.5.28 permits zero-or-more `subs` per item and
requires their `flags` to differ when more than one is present, so the
field keys the codec-specific meaning of each box. A `subsample_count`
of `0` is well-formed (empty `entries`); v0/v1 are accepted, any other
version is rejected; a truncated tuple or a non-degenerate outer table
(`entry_count != 1`, `sample_delta != 0`) is rejected; trailing bytes
past the declared sub-samples are ignored. `subs` is descriptive, so a
recognised association is surfaced typed and never trips
`Meta::unsupported_essential_properties`.

Tests: +14 unit (`subs_v0_round_trips_entries`,
`subs_v1_uses_32bit_size`, `subs_surfaces_flags`,
`subs_empty_subsample_count_ok`, `subs_rejects_entry_count_not_one`,
`subs_rejects_nonzero_sample_delta`, `subs_rejects_unknown_version`,
`subs_rejects_truncated_tuple`, `subs_tolerates_trailing_bytes`,
`subs_dispatched_through_parse_ipco`,
`subs_essential_association_is_recognised`,
`subs_lookup_via_property_for`, …).

### Round 289 — HEIF §6.5.29 `tols` target-output-layer-set item property

The §6.5.x item-property rollout skips past §6.5.28 `subs` (which is
defined by reference to ISO/IEC 14496-12's `SubSampleInformationBox`,
a variable-length per-sub-sample structure) to land §6.5.29
TargetOlsProperty, the next property fully self-contained in the HEIF
spec text. `tols` provides the output layer set index to be used as
input for the decoding process of the associated coded image item
(§6.5.29.1). For a layered coding format carrying several output layer
sets, it pins which set the decoder selects before reconstructing the
image item.

The wire layout is taken verbatim from §6.5.29.2 — an
...
without further interpretation via `Tols::target_ols_idx`.

`tols` is structurally significant beyond being one more fixed-size
descriptor: it is the **only** descriptive property in the entire
§6.5.x family that the spec *requires* to be essential —
§6.5.29.1 states `essential shall be equal to 1`. Every other
descriptive property the crate parses (`crtt` / `mdft` / `udes` /
`aebr` / … / `pano`) is "recognised regardless of the essential bit",
but for those the essential bit is unusual. For `tols` it is
mandatory. Because the parser surfaces a typed value rather than
leaving the box as `Property::Other`, a `tols`-carrying item is
recognised and does **not** trip
[`Meta::unsupported_essential_properties`] — exactly the surface a
reader without the layered-decode path needs to consult
`Tols::target_ols_idx` and route accordingly rather than rejecting the
item outright.

Forward-compatibility behaviour matches every other FullBox-headed
property parser in this module: an unknown `version` is rejected, a
body shorter than the two-byte field is rejected, and trailing bytes
past the field are ignored. Big-endian byte order is pinned by a
`tols_target_ols_idx_is_big_endian` test against a little-endian
regression (`0x0102` = 258 would surface as `0x0201` = 513).

```text
Property::Tols(Tols {
    target_ols_idx: u16,  // §6.5.29.2 (big-endian)
})
```

Tests: +8 unit (`tols_round_trip_reads_target_ols_idx`,
`tols_target_ols_idx_is_big_endian`, `tols_rejects_unknown_version`,
`tols_rejects_truncated_body`, `tols_tolerates_trailing_bytes`,
`tols_dispatched_through_parse_ipco`,
`tols_essential_association_is_recognised` pinning the
mandatory-essential recognition surface, `tols_lookup_via_property_for`
proving the end-to-end `Meta::property_for(item_id, b"tols")` lookup).
Default lib 444 (was 436); standalone lib 429 (was 421); integration
unchanged. Re-export: `oxideav_avif::Tols`.

Followups: §6.5.28 `subs` (SubSampleInformationProperty — needs the
ISO/IEC 14496-12 `SubSampleInformationBox` per-sub-sample layout, with
the §6.5.28 constraints `entry_count == 1` and `sample_delta == 0`).
§6.5.36 `amve` (AmbientViewingEnvironmentProperty) defers its syntax to
the 14496-12 `AmbientViewingEnvironmentBox`, which is **not** present
in the staged ISOBMFF docs (the checked-in 14496-12 PDFs are the
2012 / 2015 editions, predating that box) — a docs gap for that
property's body. §6.5.37 `prdi` (progressive derived image item
information), §6.5.38 `slst` single stream, §6.5.39 / §6.5.40 camera
extrinsic / intrinsic matrices remain in the staged third-edition PDF.

### Round 280 — HEIF §6.5.27 `pano` panorama-information item property

This round was dispatched against the freshly staged ISO/IEC
23008-12 third edition (2025-07,
`docs/image/heif/ISO_IEC_23008-12-HEIF-3rd-ed-2025.pdf`) to land the
`tmap` descriptor body — but a full-text sweep of the staged PDF
shows the third edition contains **no** `tmap` / tone-map / gain-map
clause at all (the §6.5.x property table tops out at §6.5.40 camera
intrinsic matrix, and §6.6.2's derived-image item types are `iden` /
`iovl` / `grid` only). The `tmap` descriptor body was *already*
implemented here in an earlier round from ISO 21496-1:2025 Annex C.2
(`GainMapMetadata::parse` + the `gain_map_metadata` extractor), so
nothing was missing in the first place. Per the dispatch's pivot
rule, the round lands the next followup the staged docs fully
ground: §6.5.27 PanoramaProperty.

The `pano` descriptive item property defines the characteristics of
a panorama declared by a `'pano'` entity group (§6.8.8.1): the type
of panorama and the scanning order of the input images. Per
§6.5.27.1 quantity is at-most-one and the property `should` only be
associated with an entity group whose `grouping_type` is `'pano'`.

The wire layout is taken verbatim from §6.5.27.2 — a
...
This is the first **conditionally sized** body in the §6.5.x
rollout: the two grid-shape bytes exist on the wire only when the
direction signals a grid panorama. The parser mirrors that shape
directly — `Pano::grid: Option<PanoGrid>` is `Some` exactly when
`panorama_direction ∈ {4, 5}`, a truncated grid body is rejected,
and a linear body ignores trailing forward-compat bytes like every
other FullBox-headed property parser in the module.

Per §6.5.27.3 the six defined directions (0 left-to-right, 1
right-to-left, 2 bottom-to-top, 3 top-to-bottom, 4 grid raster-scan,
5 grid continuous/boustrophedon) are exposed as `Pano::DIRECTION_*`
constants with `is_defined_direction()` / `is_grid()` projections.
"Other values are undefined" — an undefined direction (`>= 6`)
parses cleanly with the raw byte preserved so a reader can skip the
panorama reconstruction without losing the rest of the file. Both
grid extents are stored minus-one; `PanoGrid::{rows, columns}` add
the one back with a `u16` widening so the `255` wire endpoint reads
as `256` instead of wrapping.

The companion §6.8.8.1 grouping recogniser
`EntityGroup::is_panorama()` joins `is_alternates` /
`is_stereo_pair` / `is_equivalence`, classifying the entity group
whose increasing-entity order the `pano` direction describes. A
recognised `pano` does not trip
`Meta::unsupported_essential_properties`.

Tests: +11 (10 meta — round-trip both shapes, all six defined
directions, undefined-direction preservation, `u16` widening at the
`255` endpoint, version/truncation rejection on both shapes,
trailing-byte tolerance, `parse_ipco` dispatch, essential-bit
recognition, `property_for` lookup; 1 derived — `pano` entity-group
classification): standalone lib 421 (was 410); integration 61 + 1
ignored unchanged. Re-exports: `oxideav_avif::{Pano, PanoGrid}`.

Followups: §6.5.36 `amve` (AmbientViewingEnvironmentProperty — the
HDR-rendering hint alongside the `mdcv` / `clli` family). §6.5.37
`prdi` (progressive derived image item information). §6.5.39 /
§6.5.40 camera extrinsic / intrinsic matrices. The §6.8.8.2 `pano`
*sample group* (image-sequence side) would slot into the AVIS
surface.

### Round 271 — HEIF §6.5.26 `dobr` depth-of-field-information item property

The descriptive item-property rollout picks up §6.5.26
DepthOfFieldProperty, the depth-of-field-bracketing sibling of
`aebr` (auto-exposure) / `wbbr` (white-balance) / `fobr` (focus) /
`afbr` (flash exposure): it carries the depth-of-field variation
applied to the associated image item, relative to the camera
settings, expressed as an **aperture change** in a number of stops
as a rational. Per §6.5.26.1 the property is descriptive with
`Quantity (per item): At most one`, matching the rest of the
camera-bracketing family — a single item carries zero or one
`dobr` instance, and the property is used in conjunction with a
`dobr` entity group (§6.8.6) to identify the relative position of
a frame inside a depth-of-field-bracketed burst.

The wire layout is taken verbatim from §6.5.26.2 — a
FullBox(`dobr`, version=0, flags=0) followed by two equal-width
**signed** 8-bit fields:
...
aperture change in a number of stops, computed as the ratio
`f_stop_numerator / f_stop_denominator`. The signed interpretation
is load-bearing for downstream consumers because a
depth-of-field-bracketed burst carries both directions: a negative
numerator carries an aperture change toward a shallower depth of
field relative to the camera-set aperture, and a positive numerator
toward a deeper depth of field. A `dobr_fields_are_signed` unit
test pins this against an `as u8` regression by walking
single-sign-negative, double-sign-negative, the `i8::MIN` /
`i8::MAX` endpoints, and a raw `0xFF` byte that must read as `-1`,
not `255`.

The wire shape is structurally identical to the §6.5.25 `afbr`
flash-exposure sibling. Like `afbr` (and unlike `fobr`'s §6.5.24.3
divide-by-zero infinity sentinel), §6.5.26 does **not** carve out a
dedicated sentinel for a zero denominator — a zero denominator is
mathematically undefined, so the `Dobr::aperture_stops` projection
returns `None` in that case. The `i8::MIN / -1` corner — which
would overflow an integer-only divide — round-trips as `128.0` via
the explicit `f64::from` widening, ruling out an arithmetic panic. A
zero-numerator non-zero-denominator reading is the canonical "no
aperture variation relative to the camera setting" case and
surfaces as `Some(0.0)`.

```text
Property::Dobr(Dobr {
    f_stop_numerator:   i8,  // §6.5.26.2 (signed)
    f_stop_denominator: i8,  // §6.5.26.2 (signed)
})

Dobr::aperture_stops() -> Option<f64>   // None on den == 0
```

Forward-compatibility behaviour matches every other FullBox-headed
property parser in this module: an unknown `version` is rejected, a
body shorter than the two-byte fixed tail is rejected (covering a
header-only buffer and a header + the numerator alone), and trailing
bytes past the two-byte tail are ignored.

A recognised `dobr` property — even when flagged essential in the
...
`wbbr` / `fobr` / `afbr` properties on the always-honoured list.
(`dobr` is descriptive per §6.5.26.1, so the §7 grid-derivation
audit is untouched — transformative-property scope only.)

Test delta: +9 unit
(`dobr_round_trip_reads_numerator_then_denominator`,
`dobr_fields_are_signed`, `dobr_aperture_stops_projection`,
`dobr_rejects_unknown_version`, `dobr_rejects_truncated_body`,
`dobr_tolerates_trailing_bytes`, `dobr_dispatched_through_parse_ipco`,
`dobr_essential_association_is_recognised`, and
`dobr_lookup_via_property_for` proving the end-to-end
`Meta::property_for(item_id, &DOBR)` lookup plus a `dobr`/`afbr`
co-occurrence on a single item). Default lib 425 (was 416);
standalone lib 410 (was 401); integration 61 + 1 ignored unchanged.
Re-exports: `oxideav_avif::Dobr`.

This brings the §6.5.x typed-property coverage to every descriptive
property from §6.5.4 (`pasp`) through §6.5.26 (`dobr`), with the
camera-side bracketing family now complete: exposure (`aebr`), white
balance (`wbbr`), focus (`fobr`), flash exposure (`afbr`), and depth
of field (`dobr`).

Followups: §6.5.27 `pano` (PanoramaProperty — characteristics of a
panorama defined by a `pano` entity group). §6.5.36 `amve`
(AmbientViewingEnvironmentProperty) is the HDR-rendering hint
pulling from CTA-861.3 and would slot in alongside the
`mdcv` / `clli` family.

### Round 263 — HEIF §6.5.25 `afbr` flash-exposure-information item property

The descriptive item-property rollout picks up §6.5.25
FlashExposureProperty, the flash-bracketing sibling of `aebr`
(auto-exposure) / `wbbr` (white-balance) / `fobr` (focus): it
carries the flash exposure variation applied to the associated
image item, relative to the camera settings, as a number of
f-stops expressed as a rational. Per §6.5.25.1 the property is
descriptive with `Quantity (per item): At most one`, matching the
rest of the camera-bracketing family — a single item carries zero
or one `afbr` instance, and the property is used in conjunction
with an `afbr` entity group (§6.8.6) to identify the relative
position of a frame inside a flash-bracketed burst.

The wire layout is taken verbatim from §6.5.25.2 — a
FullBox(`afbr`, version=0, flags=0) followed by two equal-width
**signed** 8-bit fields:
...
in number of f-stops as the ratio `flash_exposure_numerator /
flash_exposure_denominator`. The signed interpretation is
load-bearing for downstream consumers because a flash-bracketed
burst routinely carries both directions: a negative numerator
carries an under-exposed (darker) flash position relative to the
camera-set flash exposure, and a positive numerator an
over-exposed (brighter) position. An `afbr_fields_are_signed`
unit test pins this against an `as u8` regression by walking
single-sign-negative, double-sign-negative, the `i8::MIN` /
`i8::MAX` endpoints, and a raw `0xFF` byte that must read as
`-1`, not `255`.

Unlike `fobr`'s §6.5.24.3 divide-by-zero infinity sentinel,
§6.5.25 does **not** carve out a dedicated sentinel for a zero
denominator — a zero denominator is mathematically undefined, so
the `Afbr::flash_exposure_stops` projection returns `None` in
that case (mirroring the `aebr` / `Aebr::exposure_stops` pattern
on the reserved zero step). The `i8::MIN / -1` corner — which
would overflow an integer-only divide — round-trips as `128.0`
via the explicit `f64::from` widening, ruling out an arithmetic
panic. A zero-numerator non-zero-denominator reading is the
canonical "no flash variation relative to the camera setting"
case and surfaces as `Some(0.0)`.

```text
Property::Afbr(Afbr {
    flash_exposure_numerator:   i8,  // §6.5.25.2 (signed)
    flash_exposure_denominator: i8,  // §6.5.25.2 (signed)
})

Afbr::flash_exposure_stops() -> Option<f64>   // None on den == 0
```
...
two-byte fixed tail is rejected (the truncation check covers a
header-only buffer and a header + the numerator alone), and
trailing bytes past the two-byte tail are ignored so a v0
producer that pads the box with reserved bytes for a future spec
revision is read cleanly.

A recognised `afbr` property — even when flagged essential in the
`ipma` association — does not trip
[`Meta::unsupported_essential_properties`], joining the previously
recognised `clap` / `irot` / `imir` / `lsel` / `a1op` / `a1lx` /
`iscl` / `rref` / `crtt` / `mdft` / `udes` / `altt` / `aebr` /
`wbbr` / `fobr` properties on the always-honoured list. (`afbr`
is descriptive per §6.5.25.1, so the §7 grid-derivation audit is
untouched — transformative-property scope only.)

Test delta: +9 unit
(`afbr_round_trip_reads_numerator_then_denominator`,
`afbr_fields_are_signed` covering single-sign-negative,
double-sign-negative, the `i8::MIN` / `i8::MAX` endpoints, and a
raw `0xFF` byte interpretation,
`afbr_flash_exposure_stops_projection` walking `+0.5` / `-0.5` /
`+1.0` / `-2/3` stops, the `i8::MIN / -1 = +128` widening
endpoint, the zero-denominator undefined reading, the
zero-numerator `0/N` zero-stops reading, and the `0/0` undefined
reading, `afbr_rejects_unknown_version`,
`afbr_rejects_truncated_body` covering both truncation shapes
(header-only and header + numerator only),
`afbr_tolerates_trailing_bytes`,
`afbr_dispatched_through_parse_ipco`,
`afbr_essential_association_is_recognised`, and
`afbr_lookup_via_property_for` proving the end-to-end
`Meta::property_for(item_id, &AFBR)` lookup for a well-formed
`+0.5`-stop reading, a `-0.75`-stop negative-bracket reading,
and the zero-denominator undefined reading). Default lib 416
(was 407); standalone lib 401 (was 392); integration 61 + 1
ignored unchanged. Re-exports: `oxideav_avif::Afbr`.

This brings the §6.5.x typed-property coverage to every
descriptive property from §6.5.4 (`pasp`) through §6.5.25
(`afbr`), with the camera-side bracketing family now covering
exposure (`aebr`), white balance (`wbbr`), focus (`fobr`), and
flash exposure (`afbr`).

Followups: §6.5.26 `dobr` (DepthOfFieldProperty — same signed
`int(8)` ratio shape as `afbr` but expressing the depth-of-field
aperture change as a number of f-stops). §6.5.36 `amve`
(AmbientViewingEnvironmentProperty) is the HDR-rendering hint
pulling from CTA-861.3 and would slot in alongside the
`mdcv` / `clli` family.
