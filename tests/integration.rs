...
//! and drive `AvifDecoder::send_packet` end to end: the AV1 pixel
//! decode is delegated to `oxideav_av1`'s conformance-grade
//! spec-driver decoder, so every committed fixture is required to
//! decode to real pixels — the historical "accept `Unsupported` when
//! the AV1 decoder declines" tolerance is retired.
//!
...
/// return the decoded `VideoFrame`. The AV1 pixel decode is delegated
/// to `oxideav_av1`'s conformance-grade registry decoder, so every
/// committed fixture is REQUIRED to decode — the historical
/// "skip on AV1-decoder limitation" tolerance (workspace task #765 /
/// the 2026-05-20 orphan-rebuild stub window) is retired: any decode
/// error is a regression and panics with the supplied label.
fn decode_video(label: &str, bytes: &[u8]) -> oxideav_core::frame::VideoFrame {
...
/// `Decoder::send_packet` trait must take the sequence path and
/// surface every decoded sample as a `Frame::Video` on the `pending`
/// queue. The `alpha_video.avif` fixture's sequence track decodes
/// fully through the `oxideav_av1` spec driver (inter frames
/// included), so every sample must come back as a video frame.
#[test]
fn avis_decode_dispatches_to_sequence_path() {
    let mut d = AvifDecoder::new(CodecId::new(oxideav_avif::CODEC_ID_STR));
    let pkt = Packet::new(0, TimeBase::new(1, 1), ALPHA_VIDEO_AVIS.to_vec());
    d.send_packet(&pkt).expect("AVIS sequence decode");
    ...
    loop {
        match d.receive_frame() {
            ...
            Err(e) => panic!("receive_frame failed after {frames} frames: {e}"),
        }
    }
    // The track's sample table drives the count; the fixture carries
    // dozens of samples and every one must decode.
    assert!(frames >= 30, "expected the full sequence, got {frames}");
}
...
#[test]
fn decode_avis_file_returns_full_frame_count() {
    let mut d = AvifDecoder::new(CodecId::new(oxideav_avif::CODEC_ID_STR));
    let n = d
        .decode_avis_file(ALPHA_VIDEO_AVIS)
        .expect("decode_avis_file");
    assert!(n >= 30, "expected the full sequence, got {n}");
...
/// Every real-world fixture — the 1280×720 monochrome still, the
/// 3840×2160 4:2:0 + alpha `bbb_alpha`, and the irot-carrying
/// 1024×722 `kimono_rotate90` — decodes END TO END through the
/// `oxideav_av1` spec-driver hand-off, and the emitted frame's
/// geometry matches the `ispe` dims rotated by the file's `irot`.
/// (The historical "accept `Unsupported`" tolerance from the
/// partial-decoder era is retired: these fixtures decoding is now the
/// regression gate.)
#[test]
fn real_world_fixtures_decode_end_to_end() {
    for (name, bytes, expect_alpha) in [
        ("monochrome", MONO, false),
        ("bbb_alpha", BBB_ALPHA, true),
        ("kimono_rotate90", KIMONO_ROT90, false),
    ] {
        let mut d = AvifDecoder::new(CodecId::new(oxideav_avif::CODEC_ID_STR));
        let pkt = Packet::new(0, TimeBase::new(1, 1), bytes.to_vec());
        d.send_packet(&pkt)
            .unwrap_or_else(|e| panic!("{name}: send_packet failed: {e}"));
        // Frame must match the inspect dims post-irot. `info.width` /
        // `info.height` come from the primary item's `ispe` box, which
        // records the pre-transform extent (HEIF §6.5.3). The decoder
        // applies any associated `irot` before emitting the frame, so
        // on a 90° / 270° rotation the displayed width/height swap
        // relative to ispe.
        ...
        let hdr =
            parse_header(bytes).unwrap_or_else(|e| panic!("{name}: parse_header failed: {e}"));
        let primary = hdr.meta.primary_item_id.expect("pitm");
...
fn decodes_small_fixtures_end_to_end() {
    ...
    for (name, bytes, (w, h), nplanes) in cases {
        ...
        let vf = decode_video(name, bytes);
...
// Re-enabled with `oxideav-av1`'s conformance-grade spec-driver
// decoder: the lossless flat-gray fixture now decodes to an exactly
// constant Y=128 plane (the pre-rebuild range-9 intra drift is gone).
#[test]
fn decodes_flat_gray_to_mid_value() {
    ...
    assert!(
        (mean - 128.0).abs() < f64::EPSILON,
        "lossless flat gray must decode to exactly Y=128, got mean={mean:.2} range={mn}..{mx}"
    );
    assert_eq!(
        (mn, mx),
        (128, 128),
        "lossless flat gray must be exactly constant"
    );
}
...
fn end_to_end_decode_then_irot_roundtrips() {
    ...
    // Run the round-trip on BOTH the real decode (uniform red — a
    // regression gate on the decode pipeline) and a position-hashed
    // synthetic frame (non-uniform — the strong permutation check;
    // a rotation bug can't hide behind uniform content).
    for (subject, start) in [
        ("red64", decoded),
        ("synthetic", synthetic_yuv444_frame(64, 64)),
    ] {
        let original_y = start.planes[0].data.clone();
        let mut frame: AvifFrame = start;
        ...
        for turn in 0..4 {
            ...
            assert_eq!(nh, w, "{subject} turn {turn}: height swap");
            frame = next;
            w = nw;
            h = nh;
        }
        assert_eq!(
            ...
            "{subject}: four 90° turns must round-trip Y plane byte-for-byte"
        );
    }
...
fn end_to_end_decode_then_imir_roundtrips() {
    ...
    for axis in 0u8..=1 {
        // The double-flip round-trip property is a pixel permutation
        // invariant — exercised on a non-uniform position-hashed
        // synthetic frame (the real-decode pipeline is covered by the
        // irot sibling test; uniform red would mask axis bugs here).
        let vf: AvifFrame = synthetic_yuv444_frame(64, 64);
...
/// Build an AVIF-shaped file where a metadata item uses
/// `construction_method == 2` (item_offset): item 2 is an `Exif` item
/// whose bytes are a sub-range of the primary `av01` item's data, named
/// via an `'iloc'` item reference. This drives the full public
/// `item_payload_bytes` path (ftyp + meta walk + iref + iloc) through the
/// cm=2 resolver. `sub_offset` / `sub_len` select the slice of the
/// primary OBU bytes the cm=2 extent addresses.
fn build_avif_with_cm2_item(obu: &[u8], sub_offset: u32, sub_len: u32) -> Vec<u8> {
    fn u32be(v: u32) -> [u8; 4] {
        v.to_be_bytes()
    }
    fn u16be(v: u16) -> [u8; 2] {
    ...
    // iinf: item 1 = av01 (primary), item 2 = Exif (cm=2 view).
    let infe1 = infe_v2(1, b"av01");
    let infe2 = infe_v2(2, b"Exif");
    let mut iinf_body = Vec::new();
    iinf_body.extend_from_slice(&u16be(2));
    ...
    // iref carries TWO entries:
    //   cdsc: Exif (2) describes the primary (1) — so inspect picks it up
    //         as the Exif item.
    //   iloc: Exif (2) names item 1 as its cm=2 data-origin (§8.11.3.3).
    let mut cdsc = Vec::new();
    cdsc.extend_from_slice(&u16be(2)); // from_item
    cdsc.extend_from_slice(&u16be(1)); // ref_count
    cdsc.extend_from_slice(&u16be(1)); // to_item
    let cdsc_box = box_bytes(b"cdsc", &cdsc);
    let mut iloc_ref = Vec::new();
    iloc_ref.extend_from_slice(&u16be(2)); // from_item (the cm=2 item)
    iloc_ref.extend_from_slice(&u16be(1)); // ref_count
    iloc_ref.extend_from_slice(&u16be(1)); // to_item (data origin)
    let iloc_ref_box = box_bytes(b"iloc", &iloc_ref);
    let mut iref_body = Vec::new();
    iref_body.extend_from_slice(&cdsc_box);
    iref_body.extend_from_slice(&iloc_ref_box);
    let iref = full_box(b"iref", 0, 0, &iref_body);
    ...
    // iloc v1, index_size=4 so the cm=2 item carries its extent_index.
    //   item 1 (av01): cm=0, single extent into mdat.
    //   item 2 (Exif): cm=2, single extent — extent_index=1, offset =
    //                  sub_offset, length = sub_len into item 1's data.
    // Per-item v1 header: id(2) cm(2) data_ref(2) extent_count(2).
    // cm=0 extent (index_size=4): extent_index(4) offset(4) length(4)=12.
    // cm=2 extent: extent_index(4) offset(4) length(4)=12.
    // item block = 8 (header) + 12 (extent) = 20; two items = 40.
    let iloc_header = 1 + 3 + 1 + 1 + 2; // version+flags + 2 size bytes + count
    let iloc_size = 8 + iloc_header + 40;
    let ftyp_size = ftyp.len();
    let meta_payload_size =
        4 + hdlr.len() + pitm.len() + iinf.len() + iref.len() + iprp.len() + iloc_size;
    let meta_size = 8 + meta_payload_size;
    ...
    let mut iloc_inner = Vec::new();
    iloc_inner.push(0x44); // offset_size=4, length_size=4
    iloc_inner.push(0x04); // base_offset_size=0, index_size=4
    iloc_inner.extend_from_slice(&u16be(2)); // item_count
                                             // item 1 av01 cm=0
    iloc_inner.extend_from_slice(&u16be(1));
    iloc_inner.extend_from_slice(&u16be(0)); // cm=0
    iloc_inner.extend_from_slice(&u16be(0)); // data_ref
    iloc_inner.extend_from_slice(&u16be(1)); // extent_count
    iloc_inner.extend_from_slice(&u32be(0)); // extent_index (unused for cm=0)
    iloc_inner.extend_from_slice(&u32be(mdat_payload_start as u32));
    iloc_inner.extend_from_slice(&u32be(obu.len() as u32));
    // item 2 Exif cm=2 (item_offset)
    iloc_inner.extend_from_slice(&u16be(2));
    iloc_inner.extend_from_slice(&u16be(2)); // cm=2
    iloc_inner.extend_from_slice(&u16be(0)); // data_ref (unused)
    iloc_inner.extend_from_slice(&u16be(1)); // extent_count
    iloc_inner.extend_from_slice(&u32be(1)); // extent_index = 1 (the 'iloc' ref)
    iloc_inner.extend_from_slice(&u32be(sub_offset)); // offset into item 1 data
    iloc_inner.extend_from_slice(&u32be(sub_len)); // length (0 = whole item)
    let iloc = full_box(b"iloc", 1, 0, &iloc_inner);
...
/// End-to-end: `item_payload_bytes` resolves a `construction_method == 2`
/// (item_offset) metadata item — item 2's bytes are a sub-range of the
/// primary `av01` item, named through an `'iloc'` item reference
/// (ISO/IEC 14496-12 §8.11.3.3). Drives the public parse path, not the
/// resolver helper directly.
#[test]
fn cm2_item_offset_metadata_resolves_via_public_path() {
    use oxideav_avif::item_payload_bytes;
    // A recognisable primary OBU payload; item 2 will view bytes 3..9.
    let obu: Vec<u8> = (0u8..16).collect();
    let file = build_avif_with_cm2_item(&obu, 3, 6);

    // The primary av01 still parses and surfaces via inspect; the Exif
    // describes-relationship is detected.
    let info = inspect(&file).expect("inspect cm=2 file");
    assert_eq!((info.width, info.height), (8, 8));
    assert_eq!(info.exif_item_id, Some(2));

    // The cm=2 item resolves to exactly the addressed slice of item 1.
    let got = item_payload_bytes(&file, 2).expect("resolve cm=2 item");
    assert_eq!(got, (3u8..9).collect::<Vec<u8>>());

    // The primary item itself still resolves to the full OBU.
    let primary = item_payload_bytes(&file, 1).expect("resolve primary");
    assert_eq!(primary, obu);
}

/// A cm=2 extent with length 0 addresses the *entire* referenced item.
#[test]
fn cm2_item_offset_zero_length_is_whole_item() {
    use oxideav_avif::item_payload_bytes;
    let obu: Vec<u8> = (100u8..120).collect();
    let file = build_avif_with_cm2_item(&obu, 0, 0); // offset 0, length 0
    let got = item_payload_bytes(&file, 2).expect("resolve whole-item cm=2");
    assert_eq!(got, obu);
}
